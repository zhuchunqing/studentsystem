...
import type { PageQuery, PageResult, Score, ScoreBatchRequest, ScoreStatistics, StudentScoreResponse } from '@/types'
...
export function createScore(data: Partial<Score>): Promise<Score> {
...
export function updateScore(id: number, data: Partial<Score>): Promise<Score> {
  return put<Score>(`/scores/${id}`, data)
}

export function auditScore(id: number, approved: boolean): Promise<Score> {
  return put<Score>(`/scores/${id}/audit`, { approved })
}
